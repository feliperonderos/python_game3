...
namespace Tree
{
    public class Environment : Node {
        ...
        public Environment(Dictionary<string, Node> d, Environment next)
        {
            dict = d;
            nextLayer = next;
        }
        public Environment(Environment e2) {
            nextLayer = e2;
            dict = new Dictionary<string, Node>();
        }
        public void change(string s, Node n) {
            if (dict.ContainsKey(s)) {
                dict[s] = n;
            }
        }
        public void define(string s, Node n)
        {
            dict[s] = n;
        }
        ...
        public Environment duplicate() {
            if (nextLayer != null)
            {
                return new Environment(
                    new Dictionary<string, Node>(dict),
                    nextLayer.duplicate()
                    );
            }
            else {
